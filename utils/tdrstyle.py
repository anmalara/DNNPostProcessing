...
def SetAlternative2DColor(h=None, tdrStyle=None):
...
def tdrCanvas(canvName, x_min, x_max, y_min, y_max, nameXaxis, nameYaxis, square=kRectangular, iPos=11, is2D=False, isExtraSpace=False):
  ...
  canv.SetLeftMargin( (B/W+0.04 if isExtraSpace else B/W) if is2D else L/W+0.02)
  canv.SetRightMargin( (B/W+0.02 if isExtraSpace else B/W+0.01) if is2D else R/W+0.01)
